...
class Property(List):
    ...
    def copy(self):
        nvalues = [v for v in self]
        return Property(self.id, nvalues, self.name)
...
class Cursor:
    ...
    def pushNode(self, node):
        var = GameTree([node])
        self.gametree.pushTree( var, self.index )
        self._setChildren()
        self._setFlags()
    ...
    def appendNode(self, node):
        if self.index + 1 < len(self.gametree) or self.gametree.variations:
            var = GameTree([node])
            self.gametree.appendTree( var, self.index )
            self._setChildren()
            self._setFlags()
        else:
